...
namespace LogiqApps
{
    public class PluginBase : IPlugin
    {
        ...
        public class LocalPluginContext
        {
            ...
            {
                if (!(this.PluginExecutionContext.InputParameters?.Contains(parameterName) ?? false))
                {
                    return default(T);
                }
            ...
            public T GetOutput<T>(string parameterName)
            {
                ...
                if (!this.PluginExecutionContext.OutputParameters.Contains(parameterName))
                {
                    return default(T);
                }

                var param = this.PluginExecutionContext.OutputParameters[parameterName];
            ...
            public void SetOutput(string parameterName, object value)
            {
                if (this.PluginExecutionContext.OutputParameters.Contains(parameterName))
                {
                    this.PluginExecutionContext.OutputParameters[parameterName] = value;
                }
                else
                {
                    this.PluginExecutionContext.OutputParameters.Add(parameterName, value);
                }
            }
            ...
            public void SetOutput(OrganizationResponse response)
            {
                ...
                {
                    if (this.PluginExecutionContext.OutputParameters.Contains(item.Key))
                    {
                        this.PluginExecutionContext.OutputParameters[item.Key] = item.Value;
                    }
                    else
                    {
                        this.PluginExecutionContext.OutputParameters.Add(item.Key, item.Value);
                    }
                }
            }
            ...
            public T PreImage<T>(string imageName = "PreImage") where T : Entity => (this.PluginExecutionContext.PreEntityImages?.Contains(imageName) ?? false) ? this.PluginExecutionContext.PreEntityImages[imageName].ToEntity<T>() : null;
            ...
            public LocalPluginContext(IServiceProvider serviceProvider)
            {
                ...
                this._target = this.GetNullAbleInput<object>("Target");
            }
